...
export async function GET() {
  try {
    const {data} = await avalaiJson<unknown>('/balance', {method: 'GET', useUserBase: true});
    return Response.json(data);
  } catch (error) {
    if (error instanceof AvalaiHttpError && error.status === 404) {
      return jsonError('Balance endpoint not found. // TODO(DOCS): confirm endpoint path.', 404);
    }
