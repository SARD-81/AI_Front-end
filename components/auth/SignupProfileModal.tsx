        ...
        <Form {...form}>
          <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
            ...
            <section className="space-y-5 rounded-3xl border border-white/10 bg-white/[0.05] p-4 shadow-inner shadow-white/[0.02] sm:p-6">
              ...
              {currentStep === 'role' ? <FormField control={form.control} name="role" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.roleLabel')}</FormLabel><FormControl><div className="grid gap-3 sm:grid-cols-2">{(['professor', 'staff'] as const).map((option) => <button key={option} type="button" onClick={() => field.onChange(option)} className={`rounded-2xl border p-4 text-start transition hover:border-primary/60 hover:bg-primary/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-field-focus ${field.value === option ? 'border-primary/70 bg-primary/20 text-white shadow-[0_0_18px_rgba(59,130,246,0.18)]' : 'border-white/15 bg-white/[0.07] text-slate-100 hover:text-white'}`}><span className="block text-sm font-bold">{option === 'professor' ? t('signup.professorRoleLabel') : t('signup.staffRoleLabel')}</span><span className="mt-1 block text-xs leading-5 text-slate-300">{option === 'professor' ? t('signup.roleDescriptions.professor') : t('signup.roleDescriptions.staff')}</span></button>)}</div></FormControl><FormMessage /></FormItem>} /> : null}
              {currentStep === 'personal' ? <div className="grid gap-4 sm:grid-cols-2"><FormField control={form.control} name="firstName" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.firstNameLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="lastName" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.lastNameLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /></div> : null}
              {currentStep === 'academic' && role === 'student' ? <div className="grid gap-4 sm:grid-cols-2"><FormField control={form.control} name="studentId" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.studentIdLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="faculty" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.facultyLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="major" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.majorLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="degreeLevel" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.degreeLevelLabel')}</FormLabel><FormControl><select {...field} className={selectClassName}><option value="">{t('signup.selectOption')}</option>{degreeOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="entryYear" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.entryYearLabel')}</FormLabel><FormControl><Input {...field} value={field.value ?? ''} onChange={(event) => field.onChange(event.target.value ? Number(event.target.value) : undefined)} type="number" inputMode="numeric" className={inputClassName} /></FormControl><FormMessage /></FormItem>} /></div> : null}
              {currentStep === 'academic' && (role === 'professor' || role === 'staff') ? <div className="grid gap-4 sm:grid-cols-2"><FormField control={form.control} name="personnelId" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.personnelIdLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="faculty" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.facultyLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="department" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.departmentLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} />{role === 'professor' ? <FormField control={form.control} name="academicRank" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.academicRankLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} /> : <FormField control={form.control} name="jobTitle" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.jobTitleLabel')}</FormLabel><FormControl><Input {...field} className={inputClassName} /></FormControl><FormMessage /></FormItem>} />}</div> : null}
              {currentStep === 'password' ? <div className="grid gap-4 sm:grid-cols-2"><FormField control={form.control} name="password" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.passwordLabel')}</FormLabel><FormControl><Input {...field} type="password" autoComplete="new-password" className={inputClassName} /></FormControl><FormMessage /></FormItem>} /><FormField control={form.control} name="confirmPassword" render={({field}) => <FormItem><FormLabel className="text-slate-200/90">{t('signup.confirmPasswordLabel')}</FormLabel><FormControl><Input {...field} type="password" autoComplete="new-password" className={inputClassName} /></FormControl><FormMessage /></FormItem>} /></div> : null}
                </motion.div>
              </AnimatePresence>
            </section>
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-white/10 bg-slate-950/90 px-5 py-4 sm:flex-row sm:px-7">
              <Button type="button" variant="ghost" className="text-sky-100 hover:bg-white/10 hover:text-white disabled:text-slate-400" onClick={() => { setDirection(-1); setStepIndex((value) => Math.max(value - 1, 0)); }} disabled={busy || stepIndex === 0}>{t('signup.back')}</Button>
              {isLastStep ? <Button type="submit" className="flex-1 bg-primary text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/90" disabled={busy || form.formState.isSubmitting}>{busy || form.formState.isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{busy || form.formState.isSubmitting ? t('signup.completingRegistration') : t('signup.complete')}</Button> : <Button type="button" className="flex-1 bg-primary text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/90" onClick={goNext} disabled={busy}>{t('signup.next')}</Button>}
            </div>
          </form>
        </Form>
      </DialogContent>
