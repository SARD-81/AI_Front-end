...
const chipClassName = 'inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.07] px-3 py-1 text-xs font-semibold text-slate-100';
      ...
      <DialogContent dir={locale === 'fa' ? 'rtl' : 'ltr'} className="flex max-h-[92vh] w-[calc(100vw-1.5rem)] max-w-2xl flex-col overflow-hidden rounded-[1.5rem] border-white/15 bg-slate-950/95 p-0 text-white shadow-2xl backdrop-blur-xl sm:rounded-[2rem]">
        ...
        <Form {...form}>
          <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
            ...
            {registerError ? <p className="mt-4 rounded-2xl border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive-foreground">{registerError}</p> : null}
            </div>
