...
- **Phone auth.** Do not release until `PHONE_AUTH_ENABLED=true` and the pilot contract in `docs/PHONE_AUTH_V1.md` are both deployed. The flag-off page says sign-in is unavailable; it is not an email-login fallback. SMS is not required for the pilot. A confirmed public support contact is still missing. The `user_id=null` model path is still open.
