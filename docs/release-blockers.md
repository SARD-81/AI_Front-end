...
- **Phone auth.** Leave `PHONE_AUTH_ENABLED=false` until the pilot contract in `docs/PHONE_AUTH_V1.md` is deployed and checked. The pilot does not send SMS: `verification_required=false` registers directly, and `verification_required=true` is the later SMS mode. A public support phone or address is still a delivery dependency; the support step must not invent one. The `user_id=null` model path is still open. The older email registration routes remain in the tree and are not a pilot sign-in option.
