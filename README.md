...
- **مدیریت تعاملات هوش مصنوعی:** ارسال پیام روی WebSocket بعد از صدور ticket از `POST /api/app/chat/ws-ticket` انجام می‌شود. تاریخچه از BFF خوانده می‌شود. کدهای OpenRouter برای دمو نگه داشته شده‌اند و مسیر تولیدی نیستند.
- **احراز هویت سها:** ورود و ثبت‌نام با شماره موبایل و مدیریت نشست با کوکی‌های HttpOnly؛ در پایلوت ثبت‌نام بدون پیامک فقط با اعلام صریح `verification_required=false` از سرور انجام می‌شود و پس از فعال شدن سرویس SMS، مسیر کد تأیید به‌صورت قراردادی فعال خواهد شد.
...
- **چت و مکالمات:** این برنچ چت را دارد. پایه بک‌اند `/api` شامل مسیرهای گفتگو، تاریخچهٔ `messages`، صدور WebSocket ticket و بازخورد پیام است. ادعای نبود چت در این برنچ معتبر نیست. گیت‌های انتشار در `docs/release-blockers.md` هستند.
