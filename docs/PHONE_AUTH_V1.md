...
The current pilot contract is [`docs/contracts/SOHA_FRONTEND_PILOT_AUTH_CONTRACT.md`](contracts/SOHA_FRONTEND_PILOT_AUTH_CONTRACT.md), copied from the product handoff. The backend review named there is `a9564e8`; the staging config note is `1a99539` (2026-09-25). Those commits are not evidence this frontend is deployed, and a green mock test is not a live integration test. Staging smoke is unconfirmed.
...
The migration form passes the **existing account email** to the backend; the pilot handoff does not require a university email domain for this endpoint. It is not public email login. The previous email flow is preserved as an inactive source patch in [`docs/legacy-auth/`](legacy-auth/README.md) for a future separately reviewed contract, not wired into the current UI or BFF.
...
- Legacy `POST /api/auth/register/*` paths may still exist on the backend. In phone-only mode the contract requires a `410 legacy_registration_disabled` response; verify this against the deployed backend instead of treating removal of frontend controls as an access control.
...
- An open WebSocket is closed in this frontend after logout, phone reset complete, or password change. The server does not close it.
...
The unified UI branch contains both chat and pilot-auth design. The corrective branch `fix/soha-pilot-auth-identity-v2` starts at PR #132 head `998eb8a`. This note replaces the earlier phone-only branch description; see Git history and [`docs/legacy-auth/`](legacy-auth/README.md) for the disabled email-flow source.

## Earlier screenshots

The current identity panel and pilot registration preview are in [`docs/auth-identity-preview/`](auth-identity-preview/README.md). These are local Chromium captures, not staging or a physical phone.
