...
const dataFile = path.join(dataDir, 'app-chats.json');
...
function persistStore(next: StoreData): Promise<void> {
...
export async function listChats(): Promise<AppChat[]> {
  const store = await loadStore();
  return [...store.chats].sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt));
}

export async function createChat(title: string): Promise<AppChat> {
  const store = await loadStore();
...
export async function getChat(chatId: string): Promise<{chat: AppChat; messages: AppMessage[]} | null> {
  const store = await loadStore();
...
export async function renameChat(chatId: string, title: string): Promise<AppChat | null> {
  const store = await loadStore();
  const chat = store.chats.find((item) => item.id === chatId);
  if (!chat) {
    return null;
...
export async function deleteChat(chatId: string): Promise<boolean> {
  const store = await loadStore();
  const previousLength = store.chats.length;
  store.chats = store.chats.filter((item) => item.id !== chatId);

  if (store.chats.length === previousLength) {
    return false;
  }
